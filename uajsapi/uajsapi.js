...
function collectUaInfo(uainfo){
    ...
    var android=new OSRegxpRule("android",new Array("Android (.*?);"),new Array(";.*;( .*) Build\/"),1);
    var ios=new OSRegxpRule("ios",new Array("OS (.*) like Mac OS X"),new Array("\\((.*?);"),1); 
    var wp=new OSRegxpRule("windows phone",new Array("Windows Phone (.*?);"),new Array(".*; (.*?)\\)"),10);
    var windows_destop=new OSRegxpRule("windows destop",new Array("Windows NT (.*?); "),new Array(""),1);
    var macos=new OSRegxpRule("macosx",new Array("Intel Mac OS X (.*?)\\)"),new Array(""),1);
    var xwindow=new OSRegxpRule("xwindow",new Array(" \\((X11); Linux "),new Array(""),1);
    var osregxps=new Array(android,ios,wp,windows_destop,macos,xwindow);
   
     osregxps.sort(function(a,b){
    ...
    for(var i=0;i<osregxps.length;i++){
       var osregxp=osregxps[i];
        ...
        if(osver.length>0){
            ...
            uainfo.deviceinfo=uainfo.deviceinfo.toLowerCase();
            break;
        }
    }
    ...
    var appregxps=new Array(new AppRegxp("wechat","微信",new Array("MicroMessenger/(.*?) "),10),
        new AppRegxp("weibo","微博手机端",new Array("weibo__(.*?)__"),1),
        new AppRegxp("alipay","支付宝手机端",new Array("AlipayClient/(.*?) "),2),
        new AppRegxp("qq","qq手机客户端",new Array("QQ/(.*?) "),3),
        new AppRegxp("lbbrowser","猎豹浏览器",new Array(" (LBBROWSER)"),9),
        new AppRegxp("firefox","火狐浏览器",new Array(" Firefox/(\\d+\.\\d+) ","Firefox/(.*);","Firefox/(.*)"),1),
        new AppRegxp("ucbrowser","UC浏览器",new Array("UCBrowser/(.*?) "),1),
        new AppRegxp("mqqbrowser","QQ浏览器(移动端)",new Array("MQQBrowser/(.*?) "),2),
        new AppRegxp("baidubrowser","百度浏览器",new Array("baidubrowser/(.*?) "),2),
        ...
        new AppRegxp("miuibrowser","小米手机浏览器",new Array("MiuiBrowser/(.*)"),2),
        new AppRegxp("chrome","chrome",new Array("Chrome/(.*?) "),1),
        new AppRegxp("edge","Edge",new Array("Edge/(.*)"),2),
        new AppRegxp("chitu","赤兔",new Array("Chitu/(.*?) "),2),
        new AppRegxp("dingtalk","钉钉",new Array("AliApp\\(DingTalk/(.*?)\\)"),2),
        new AppRegxp("sogouexplorer","搜狗浏览器",new Array("SE (.*) MetaSr"),2),
        new AppRegxp("tbapp","淘宝app",new Array("AliApp\\(TB/(.*)\\)"),2)
...
function OSRegxpRule(osname,osverregxp,deviceinforegxp,prio){
    this.osname=osname;
